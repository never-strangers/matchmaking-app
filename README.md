...
- **Batched password resets (approved users only):** apply migrations `20260422143000_password_reset_sends.sql` through `20260424120000_password_reset_batch_require_wp_user_id.sql`. **Batch 1 / batch 2** only include profiles with **`profiles.wp_user_id IS NOT NULL`** (WordPress import); native signups are excluded. Env: `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`. For **links inside reset emails**, set **`APP_BASE_URL` or `APP_URL` to production** (e.g. `https://app.thisisneverstrangers.com`) so they are not `localhost` when `NEXT_PUBLIC_APP_URL` is dev-only. Plus `EMAIL_PROVIDER` / `RESEND_API_KEY` or `ENVLOPED_API_KEY` / `EMAIL_FROM` as in `lib/email/provider.ts`.
  ...
  Uses `auth.admin.generateLink` + `lib/email/resetPasswordHtml.js`; logs each attempt to `password_reset_sends`. Batch 1/2 also **re-check `profiles.wp_user_id` in the script** so native signups are not included if the DB RPC is outdated (still apply migration `20260424120000_password_reset_batch_require_wp_user_id.sql` so SQL preview and RPC match). **Dry run** prints a numbered preview (first 15 addresses) and writes JSON under `scripts/.seed-output/password-resets-{batch}-{timestamp}.json` (includes full `emails` array on dry run).
  - **Retry failed `email_log` rows (same row updated, no deletes):** pick an anchor row UUID in Table Editor (e.g. the first successful send after a block of errors). By default the script selects `status = 'error'` with `created_at <=` the anchor’s `created_at`. Set `EMAIL_PROVIDER=enveloped` and `ENVLOPED_API_KEY` in `.env.local` (see `lib/email/provider.ts`). Supports templates: `password_reset` (new recovery link), `rsvp_confirmation`, `payment_confirmation`, `approved`, `rejected` (keys `status-approved:<profileId>` / `status-rejected:<profileId>` from admin status changes).
    - Env is loaded from **project root** `.env.local` / `.env` (run commands from the repo folder). **`NEXT_PUBLIC_SUPABASE_URL` must be the same project** as the Table Editor where you copied the anchor id (prod vs local). For **`password_reset`** rows, set **`APP_BASE_URL` or `APP_URL`** to production so retried `generateLink` redirects to `/auth/reset-password` on the live app (same as batched password resets).
...
- Big Event (Tiered Tickets) — Early Bird / Wave 1 / Wave 2 / VIP

Each event has required fields (title, description, city, category, start_at, end_at, whats_included, payment_required, price_cents/currency). Output: `scripts/.seed-output/events.json` with event ids and types (free/paid_single/tiered).
...
- Creates a `seed_runs` row and tags data via `seed_run_id`.
- **Users per city:** 10 **approved**, 2 **pending_verification** (gated; admin review), 2 **rejected**. Profile fields: full name, dob ≥ 21, gender (incl. other/prefer_not_to_say), preferred_language (Thai/Vietnamese where relevant), instagram for some, phone.
- **Events:** Past free friends mixer; future tiered paid flagship (Early Bird/Wave1/Wave2/VIP); per city: 1 upcoming free friends mixer, 1 upcoming paid dating event.
- **Attendees** (upcoming events only):
  - **Free events:** `payment_status = 'not_required'`.
  - **Paid events:** 8 paid, 2 `checkout_created` (only paid eligible for check-in in UI).
  - **Check-in:** 8 checked in, 2 unchecked (**late arrivals** for incremental matching demo — admin can check them in after Round 1, then run matching again for Round 2).
  - **Questionnaire:** Full answers for 8 attendees (eligible for matching); 2 with 0 answers (incomplete, to test gating).
- Enough checked-in + paid + questionnaire-complete users to run **Round 1** matching (e.g. 6+ pairs). Late arrivals support **incremental matching** (run matching → reveal Round 1 → check in 1–2 late → run matching again → reveal Round 2).

**Seed does NOT create:** `match_results`, `match_rounds`, or `conversations`. Admin runs matching and reveals from the UI. Chat-now CTA is created when admin reveals a round.
...
```bash
# Full reset: clean all test-seed runs, then seed (default label "test-seed" for cleanup)
SEED_CONFIRM=true SEED_USER_PASSWORD="ChangeMe123!" npm run reset:test-data

# Dry-run cleanup
npm run cleanup:test-data -- --label "test-seed" --dry-run

# Actual cleanup by label (ILike match on seed_runs.label)
SEED_CONFIRM=true npm run cleanup:test-data -- --label "test-seed"

# Or by explicit seed_run id
