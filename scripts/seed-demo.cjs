// CommonJS version of the demo seed script so it can be run with plain Node.

// Load local env vars (NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, etc.)
...
async function main() {
  ...
  for (const row of invited) {
    const urlWithToken = `${baseUrl}/invite/${row.invite_token}`;
